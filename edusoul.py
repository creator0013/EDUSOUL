...
VERSION_URL = "https://raw.githubusercontent.com/creator0013/EDUSOUL/main/version.txt"
...
def extract_frames(video_path, output_dir):
    command = (
        f'"{ffmpeg_path}" -i "{video_path}" -vf fps=1 "{output_dir}/frame_%04d.png"'
    )
    ...
    try:
        ...
        for input_path in input_paths:
            if input_path.lower().endswith((".mp4", ".avi", ".mov", ".mkv")):
                ...
                messagebox.showinfo(
                    "Processing",
                    f"Extracting frames from video: {os.path.basename(input_path)}...",
                )
                ...
                messagebox.showinfo(
                    "Processing",
                    f"Processing image: {os.path.basename(input_path)}...",
                )
                process_image(image_path, phone_numbers, invalid_numbers)
            else:
                raise ValueError(
                    f"Unsupported file format: {os.path.basename(input_path)}. Please provide a video or image file."
                )

        # Create DataFrame with valid and invalid numbers
        data = [(num, "Valid") for num in phone_numbers] + [
            (num, "Invalid") for num in invalid_numbers
        ]
        ...
        if invalid_numbers:
            (
                messagebox.showinfo(
                    ...
                    f"{', '.join(list(invalid_numbers)[:10])}...\n"
                    f"and {len(invalid_numbers) - 10} more.",
                )
            ...
            messagebox.showinfo(
                "Success",
                f"All contact numbers processed and saved to {excel_output_path}",
            )
    ...
    file_paths = filedialog.askopenfilenames(
        filetypes=[
            (
                ...
                "*.mp4;*.avi;*.mov;*.mkv;*.png;*.jpg;*.jpeg;*.bmp;*.tiff",
            )
        ]
    )
...
def start_processing():
    ...
    if input_paths and output_path:
        ...
        messagebox.showwarning(
            "Input Error", "Please specify both input files and output Excel file."
        )
